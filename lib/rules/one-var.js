            ...
            VariableDeclaration(node) {
                ...
                function fix(fixer) {
                    return fixer.replaceTextRange(
                        [node.declarations[0].range[0], node.declarations[node.declarations.length - 1].range[0]],
                        node.declarations.slice(0, -1).reduce((replacementText, declaration, index) => {
                            const declText = sourceCode.getText(declaration);
                            const comma = sourceCode.getTokenAfter(declaration);
                            const textBeforeComma = sourceCode.text.slice(declaration.range[1], comma.range[0]);
                            const textAfterComma = sourceCode.text.slice(comma.range[1], node.declarations[index + 1].range[0]);

                            return `${replacementText}${declText}${textBeforeComma};${textAfterComma}${node.kind} `;
                        }, "")
                    );
                }
